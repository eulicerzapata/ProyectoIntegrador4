...
para el marketplace brasileño Olist durante 2016-2018.
"""
...
Explora las métricas clave de ingresos y entregas del marketplace brasileño Olist
""")
...
def show_executive_summary():
    ...
    growth_rate = ((total_2018 - total_2017) / total_2017 * 100) if total_2017 > 0 else 0
    ...
    # Columna 1: Ingresos 2018 con crecimiento vs 2017
    with col1:
        st.metric(
            label="💰 Ingresos 2018",                    # Etiqueta de la métrica
            value=f"${total_2018:,.0f}",                # Valor principal (formato con comas)
            delta=f"{growth_rate:+.1f}% vs 2017"       # Delta (cambio) con signo y color automático
        )
    
    with col2:
        st.metric(
            label="🏆 Top Categoría",
            value=categories_df.iloc[0]['Category'].replace('_', ' ').title(),
            delta=f"${categories_df.iloc[0]['Revenue']:,.0f}"
        )
    
    with col3:
        st.metric(
            label="🎯 Top Estado",
            value=states_df.iloc[0]['customer_state'],
            delta=f"${states_df.iloc[0]['Revenue']:,.0f}"
        )
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color='#1f77b4', width=3),  # Color y grosor de línea
    ...
    fig.update_layout(
        title="Ingresos Mensuales 2017-2018",    # Título del gráfico
    ...
    with col1:
        ...
        **💡 Tendencias de Ingresos:**
        - Fuerte crecimiento entre 2017 y 2018
        - Picos de ventas en Mayo-Agosto 2018
        - Caída significativa en Q4 2018
        """)
...
def show_revenue_analysis():
    ...
    with col2:
        ...
        if not least_df.empty:
            least_display = least_df.tail(5).copy()
...
def show_delivery_analysis():
    ...
    # Filtrar datos válidos para 2017 y 2018
    months = delivery_df['month'].tolist()
    
    fig = make_subplots(
        rows=2, cols=1,
        subplot_titles=('Año 2017', 'Año 2018'),
    ...
    fig.update_layout(height=700, title_text="Comparación de Tiempos de Entrega por Mes")
    ...
    with col3:
        ...
        st.metric(
            label="🎯 Precisión Estimaciones 2017",
            value=f"{100-accuracy_2017:.1f}%",
            delta="Precisión promedio"
        )
